...
export const brandAssetTypeOptions = brandAssetTypeValues.map((value) => ({
  label: brandAssetTypeLabels[value],
  value,
}));

export const brandAssetStatusOptions = brandAssetStatusValues.map((value) => ({
  label: brandAssetStatusLabels[value],
  value,
}));
...
export function parseBrandKitFormData(
  ...
  const parsedValues = brandKitFormSchema.safeParse(rawValues);
...
function parseBrandKitConfig(
  configJson: string,
): BrandKitFormParseResult | { ok: true; values: Record<string, Json> } {
  if (!configJson) {
    return {
      ok: true,
      values: {},
