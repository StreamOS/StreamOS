...
Use this when the `apps/web` dev server fails with a stale or corrupt Next.js
artifact error such as:
...
2. Reset the generated build output:
...
If only `dev` is broken but `build` succeeds, the issue is usually a stale local
cache rather than a source-code regression.
